...
async function start() {
  ...
  app.get('/api/files', (c) => { const requested = c.req.query('path'); if (!requested) return c.text('Arquivo não informado.', 400); const target = resolve(requested); if (!target.startsWith(resolve(outputDir)) || !existsSync(target)) return c.text('Arquivo não encontrado.', 404); return new Response(readFileSync(target), { headers: { 'Content-Type': 'video/mp4', 'Content-Disposition': `attachment; filename="${basename(target)}"` } }); });

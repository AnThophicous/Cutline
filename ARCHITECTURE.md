...
3. Smart Cut usa `ffmpeg -af silencedetect` e heurísticas de duração, respiro e distância mínima; os trechos ficam editáveis antes do job.
4. Cada módulo aprovado vira um intermediário cacheado por hash + parâmetros.
5. As combinações são geradas por matriz H × C × CTA. O worker tenta concatenação `-c copy`; só usa reencode quando o probe mostrar incompatibilidade.
